...
describe('Set', () => {
	...
	it.each([
		...
		(collection, given, expected) => {
			const set = Set.new<any>(collection);
	...
	it('Map a ControlOption to a string', () => {
		const set = Set.new<{ label: string; value: number }>([
			{ value: 1, label: 'First' },
	...
	it('Filters out non valid elements', () => {
		const set = Set.new<string | number>(['a', 1, 'b', 2, 'c', 3]);
	...
	it('Should walk through the set', () => {
		const set = Set.new<number>().add(1).add(2).add(3).add(4).add(5);
	...
	it('Should ensure that two sets are equal in shape and content', () => {
		const set = Set.new<number>().add(1).add(2).add(3).add(4).add(5);
		const set2 = Set.new<number>().add(1).add(2).add(3).add(4).add(5);
	...
	it('Should ensure two sets are not equals if they have different length', () => {
		const set = Set.new<number>().add(1).add(2).add(3).add(4).add(5);
		const set2 = Set.new<number>().add(1).add(2).add(3).add(4);
		expect(set.equals(set2)).toBe(false);
	});

	it('Should ensure two sets are not equals if they have different content', () => {
		const set = Set.new<number>().add(1).add(2).add(3).add(4).add(5);
		const set2 = Set.new<number>().add(1).add(2).add(3).add(4).add(6);
		expect(set.equals(set2)).toBe(false);
	});
});
